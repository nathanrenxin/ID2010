...
import chat.server.ChatMessage;
    ...
    RemoteEventListener		// So we can receive chat notifications
{
      ...
      RemoteException
  {
    ...
    LiveRef lr = ((UnicastRef) this.getRef()).getLiveRef();
    host = ((TCPEndpoint) lr.getChannel().getEndpoint()).getHost() + ":" + lr.getPort();
  }
  ...
  public void serviceAdded (ServiceDiscoveryEvent e) {
    ...
    if (sit.service instanceof ChatServerInterface) {
	  ...
	  Entry en = sit.attributeSets[j];
	  if (en instanceof Name) {
	    localName = ((Name) en).name;
	    break;
	  }
      }
	  ...
	  System.out.println ("[Added server " + remoteName + "]");
          ...
          System.out.println ("[Added server " + localName + "]");
      }
    }
  }
  ...
  public void serviceChanged (ServiceDiscoveryEvent e) {
    ...
    if (postSit.service instanceof ChatServerInterface) {
      ...
      ChatServerInterface chat = (ChatServerInterface) postSit.service;
      String remoteName = null;
      String localName = null;

      for (int j = 0; j < postSit.attributeSets.length; j++) {
	  Entry en = postSit.attributeSets[j];
	  if (en instanceof Name) {
	    localName = ((Name) en).name;
	    break;
	  }
      ...
      if (localName == null) {
	  localName = postSit.service.toString ();
      }

      try {
	  remoteName = chat.getName ();
	  System.out.println ("[Changed server " + remoteName + "]");
      }
      catch (RemoteException rex) {
          System.out.println ("[Changed server " + localName + "]");
      }
    }
  }

  /**
   * The Jii lookup cache calls this method when a found service has
  ...
  public void serviceRemoved (ServiceDiscoveryEvent e) {
    ...
    if (sit.service instanceof ChatServerInterface) {
	  ...
	  System.out.println ("[Removed server " + remoteName + "]");
          ...
          System.out.println ("[Removed server " + localName + "]");
      }
    }
  }
  ...
  public void sendToChat (String text) {
	...
	myServer.say (new ChatMessage(text, host));
      }
      catch (RemoteException rex) {
